...
This is the public-tier contract in action: the instrumentation uses
only wrapture's stable surface, a binding with a transforms_args
stage and wrapture.trace_headers(), and never touches an event.
trace_headers() returns the headers the current tree's identity
should travel as, whatever minted or parsed it, and returns nothing
when nothing is being recorded, so injection is always safe to
attempt.
"""
